...
int main(int argc, char *argv[]){
	...
	while ((c = getopt(argc, argv,"nf:r:d:a:k:D:" )) != -1){		
			switch(c){
				case 'a': data_to_add = optarg;
					  break;
				case 'n': new_file = 1; // true
					  break;
			  	case 'f': file_path = optarg;
					  break;
				case 'r': record_id = optarg;
					  break;
				case 'd': fileds_and_type = optarg;
					  break;
				case 'k': key = optarg;
					  break;
				case 'D': del = 1, record_id = optarg; //true
					  break;
	...
	if(new_file){
		...
		if(del){/*this section will delete the record specified by the -D option, in the index file*/
		...
		if(data_to_add){/*this section will append data to the speified file*/
			...
			// if it is good and no different, proceed without a Schema
		...
		Record_f* rec = parse_d_flag_input(file_path,fields_count,buffer,buf_t,buf_v,&hd.sch_d,SCHEMA_EQ);
			clean_schema(&hd.sch_d);
